...
if (isset($_GET['cv_id'])) {
    ...
    $query_cmplt = "SELECT cmplt_id, comp_id, cv_id, m_id, cdare, cmpny_in, profit, any_description FROM completed_task WHERE cv_id = $cv_id";
    $result_cmplt = mysqli_query($conn, $query_cmplt);
    if ($result_cmplt && mysqli_num_rows($result_cmplt) > 0) {
        $completed_task = mysqli_fetch_assoc($result_cmplt);
    } else {
        $completed_task = null; // No completed task data found
    }
} else {
...
<head>
    ...
    <title>ASL : View Company Data, MB : GM</title>
    <style>
...
<div class="container">
    ...
    <h3>Completed Task Information</h3>
    <?php if ($completed_task) { ?>
        <table class="table">
            <tr>
                <th>Completed Task ID</th>
                <td><?php echo $completed_task['cmplt_id']; ?></td>
            </tr>
            <tr>
                <th>Company ID</th>
                <td><?php echo $completed_task['comp_id']; ?></td>
            </tr>
            <tr>
                <th>CV ID</th>
                <td><?php echo $completed_task['cv_id']; ?></td>
            </tr>
            <tr>
                <th>Manager ID</th>
                <td><?php echo $completed_task['m_id']; ?></td>
            </tr>
            <tr>
                <th>CDare</th>
                <td><?php echo $completed_task['cdare']; ?></td>
            </tr>
            <tr>
                <th>Company Involved</th>
                <td><?php echo $completed_task['cmpny_in']; ?></td>
            </tr>
            <tr>
                <th>Profit</th>
                <td><?php echo $completed_task['profit']; ?></td>
            </tr>
            <tr>
                <th>Description</th>
                <td><?php echo $completed_task['any_description']; ?></td>
            </tr>
        </table>
    <?php } else { ?>
        <p>No completed task information found.</p>
    <?php } ?>

    <a href="completed-data.php" class="btn btn-primary">Back to Company List</a>
</div>
